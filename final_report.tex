...
\author{\textbf{Harshit Verma} (Roll No: 122CS0023) \\ \\ Under the Guidance of \\ \textbf{Dr. N. Srinivas Naik} \\ Associate Professor \\ Dept. of CSE, IIITDM Kurnool \\ \\ \textbf{Prepared for: Samsung Innovation Campus (SIC) Review}}
\date{February 2026}
...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    \textbf{SAMSUNG INNOVATION CAMPUS (SIC) PROJECT PRESENTATION} \\
    March 2026
\end{titlepage}
...
\section{Main Quantitative Results: PC-LiquidGAN + ODE-UNet Architecture}
Table~\ref{tab:main} presents the final SSIM and PSNR values for our proposed model incorporating physics loss, adaptive ODE solver, FFT spectral loss, and the \textbf{novel ODE-UNet generator architecture} which drastically reduced spatial information loss.
...
\bottomrule
\end{tabular}
\caption{Final quantitative metrics for PC-LiquidGAN ODE-UNet (Ours) across 5 domains.}
\label{tab:main}
\end{table}
...
LiquidGAN (Ablation)           & \checkmark & $\times$ & $\times$ & 0.8222 \\
                             & \checkmark & \checkmark & $\times$ & \checkmark & 0.7821 \\
\textbf{Ours (Spectral)}     & \checkmark & \checkmark & \checkmark & \checkmark & 0.8290 \\
\textbf{Ours (ODE-UNet)}     & \checkmark & \checkmark & \checkmark & \textbf{ODE-UNet} & \textbf{0.9945} \\
\bottomrule
\end{tabular}
\caption{Comparative evaluation on Agri (Tomato) dataset. The ODE-UNet architecture completely solves spatial degradation.}
\label{tab:comparative}
\end{table}

\section{Zero-Shot Cross-Domain Evaluation}
We evaluated the model trained exclusively on the base Chilli domain over the unseen Agri (Tomato) dataset without any fine-tuning:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Frequency-Domain Spectral Loss:} An FFT-based loss with a Gaussian low-frequency emphasis mask enforces that generated thermal images match the spectral signature of real thermal data. Thermal images are physically low-frequency dominant (smoothly varying temperature fields) due to heat diffusion.
    
    \item \textbf{ODE-UNet Architecture:} Replaced the heavily bottlenecked Neural ODE latent vector with a spatial \texttt{ConvODEFunc} spanning high-resolution UNet skip connections, solving mode collapse and preserving ultra-high-frequency spatial details, pushing agricultural metrics from $\sim$0.80 SSIM to 0.9945 SSIM.
\end{enumerate}

\chapter{Conclusion and Future Work}
\section{Conclusion}
This work proposes PC-LiquidGAN, a physics-informed extension of LiquidGAN for robust thermal image synthesis across five diverse domains: surveillance, biometrics, healthcare, and agriculture. Four novel contributions were made: (1) physics-aware heat diffusion loss, (2) adaptive ODE solver integration, (3) FFT-based spectral loss with low-frequency Gaussian emphasis, and (4) a spatial ODE-UNet generator.

Our exhaustive evaluation demonstrates that incorporating these physical and frequency-domain constraints significantly improves structural fidelity, thermodynamic plausibility, and zero-shot cross-domain generalization, with the full model achieving near-perfect SSIM scores (0.99+) on the agricultural and biometric datasets.
