...
To enhance readability and provide a comprehensive comparison, Table~\ref{tab:domain_analysis} summarizes the distinct impact of the Physics and Spectral constraints on each domain, correlated with their unique scene physics.

\begin{table}[htbp]
\centering
\caption{Summary of domain-specific qualitative and quantitative impacts of the loss constraints.}
\label{tab:domain_analysis}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{p{2.5cm}p{3cm}p{3cm}p{4cm}}
\toprule
\textbf{Dataset Target} & \textbf{Physics Constraint} & \textbf{Spectral Constraint} & \textbf{Domain Characteristics} \\
\midrule
\textbf{KAIST (Surveillance)} & Improved PSNR but decreased SSIM & Recovers SSIM moderately & Extreme scene variability; complex daylight/night environments \\
\textbf{Medical (Knee IR)} & Strongest positive benefit (+1.4\% SSIM) & Negative structural effect & Spatially smooth temperature patterns; low-frequency dominated \\
\textbf{CBSR (Face NIR)} & Monotonic positive improvement & Monotonic positive improvement & Well-defined, natural biological smooth gradients \\
\textbf{Agri -- Chilli} & Positive (+1.0\% SSIM) & Negative effect & Structural heat diffusion with localised high-frequency anomalies \\
\textbf{Agri -- Tomato} & Negative effect (data scarcity) & Positive (+0.7\% SSIM above baseline) & Fine-grained disease patches; dataset heavily constrained \\
\bottomrule
\end{tabular}%
}
\end{table}
