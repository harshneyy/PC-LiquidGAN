"""
train_physics.py
Re-train PC-LiquidGAN (WITH physics loss) on individual datasets.
Uses per-dataset output directories to avoid parallel training corruption.

Usage:
    python train_physics.py --dataset agri --epochs 100
    python train_physics.py --dataset kaist --epochs 100
"""
...
def main():
    p = argparse.ArgumentParser(description='PC-LiquidGAN Training (Per-Dataset)')
    p.add_argument('--dataset',    type=str, required=True,
                   choices=['kaist', 'cbsr', 'medical', 'agri', 'chilli'])
    p.add_argument('--epochs',     type=int, default=100)
    p.add_argument('--batch_size', type=int, default=32)
    p.add_argument('--ode_method', type=str, default=None,
                   choices=['dopri5', 'rk4', 'euler'],
                   help='ODE solver method (overrides config default)')
    args = p.parse_args()
    ...
    # Per-dataset output directories (no more collisions!)
    solver_tag = cfg.ODE_METHOD
    save_dir = f'./results_physics/{args.dataset}_{solver_tag}'
    ckpt_dir = f'./checkpoints_physics/{args.dataset}_{solver_tag}'
    os.makedirs(save_dir, exist_ok=True)
    os.makedirs(ckpt_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"  PC-LiquidGAN Training (WITH Physics Loss)")
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        for rgb, thermal in loader:
            ...
            g_loss = cfg.LAMBDA_ADV * l_adv + cfg.LAMBDA_REC * l_rec + l_physics
            g_loss.backward()
        ...
        n = len(loader)
        print(f"[PC-LiquidGAN] Epoch [{epoch+1:3d}/{cfg.EPOCHS}]  "
              f"D: {epoch_d/n:.4f}  G: {epoch_g/n:.4f}  "
              f"Phys: {epoch_p/n:.4f}  Time: {elapsed:.1f}s")
    ...
    print(f"\nPC-LiquidGAN Training complete for {args.dataset.upper()}!")
